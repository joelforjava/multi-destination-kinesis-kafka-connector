...
This is a variation of the Kafka-Kinesis-Connector from [AWS Labs](https://github.com/awslabs/kinesis-kafka-connector). I originally forked from AWS Lab's version, but my changes have diverged a lot from the original. However, while making tweaks, I thought some of them could be applied to the original repo. This would be pretty messy with my original setup and needing the tweaks I've made while not sending them back to the original repo. So, I created this version that will be dedicated to a Connector that can send to multiple Firehose destinations while I revert these changes from my original fork so that it can be usable with any pull requests I may want to submit. I have updated this document, where possible, that documents where it diverges from the original connector. Please note that the changes apply only to the Firehose connector and not the Kinesis Streams connector (at this time).
